...
try {
  if ($action === 'preview') {
    ...
    $w="d.work_date BETWEEN ? AND ?"; $p=[$from,$to];
  ...
  if ($action==='generate') {
    ...
    $stmt=$db->prepare("INSERT INTO payroll
      (employee_id,estate_id,period_from,period_to,plucking_pay,assignment_pay,allowances,deductions,gross,net,status)
      VALUES (?,?,?,?,?,?,?,?,?,?, 'Calculated')");
    $db->beginTransaction(); $n=0;
    foreach($rows as $r){
      $stmt->execute([(int)$r['employee_id'],($b['estate_id']?:null),$from,$to,
  ...
  if ($action==='list') {
    $st=$db->query("SELECT pr.*, e.emp_code, e.full_name FROM payroll pr JOIN employees e ON e.id=pr.employee_id ORDER BY pr.id DESC LIMIT 500");
    ok(['rows'=>$st->fetchAll()]);
  }
